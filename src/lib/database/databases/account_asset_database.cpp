...
account_asset_database::account_asset_database(const path& map_filename,
...
account_asset_result account_asset_database::get_account_asset_result(const hash_digest& hash) const
{
	const auto memory = get(hash);
    return account_asset_result(memory);
}
void account_asset_database::store(const hash_digest& hash, const asset_transfer& sp_transfer)
{
    // Write block data.
    const auto key = hash;
    const auto sp_size = sp_transfer.serialized_size();

    BITCOIN_ASSERT(sp_size <= max_size_t);
    const auto value_size = static_cast<size_t>(sp_size);

    auto write = [&sp_transfer](memory_ptr data)
    {
        auto serial = make_serializer(REMAP_ADDRESS(data));
        serial.write_data(sp_transfer.to_data());
    };
