...
//#include <bitcoin/bitcoin/chain/attachment/account/account_asset.hpp>
...
std::shared_ptr<std::vector<business_address_asset>> account_asset_database::get_unissued_assets(const short_hash& key) const
{
    auto result = get(key);
