...
class TestDIDMultiSig(MVSTestCaseBase):
    ...
    def test_0_registerdid(self):
        ...
        for i, role in enumerate(group):
            addr = role.new_multisigaddress(
                "Alice & Bob & Zac's Multisig-DID", group[:i] + group[i + 1:], 2)
    ...
    def test_1_modifydid(self):
        ...
        for i, role in enumerate(group):
            addr = role.new_multisigaddress(
                "Alice & Cindy & Zac's Multisig-DID", group[:i] + group[i + 1:], 2)
        ...
        ec, tx = mvs_rpc.change_did(
            Zac.name, Zac.password, normal_new, did_symbol)
        self.assertEqual(ec, 0, tx)
        ...
        self.assertEqual(did_address, normal_new,
        ...
        ec, tx = mvs_rpc.sign_multisigtx(
            group_new[0].name, group_new[0].password, tx, True)
        self.assertEqual(ec, 0, tx)
        Alice.mining()

        did_address = group[0].get_didaddress(did_symbol)
        self.assertEqual(did_address, addr_new,
