...
class TestAssetBoundary(MVSTestCaseBase):
    ...
    def test_0_check_asset_symbol(self):
        ...
        for char in spec_char_lst:
            ec, message = mvs_rpc.create_asset(
                Alice.name, Alice.password, Alice.asset_symbol + char, 100, Alice.did_symbol)
    ...
    def test_1_create_asset(self):
        ...
        ec, message = mvs_rpc.create_asset(
            Alice.name, Alice.password + '1', Alice.asset_symbol, 100, Alice.did_symbol)
        self.assertEqual(ec, 1000, message)

        # aasset symbol can not be empty.
        ec, message = mvs_rpc.create_asset(
            Alice.name, Alice.password, "", 100, Alice.did_symbol)
        self.assertEqual(ec, 5011, message)

        # asset symbol length must be less than 64.
        ec, message = mvs_rpc.create_asset(
            Alice.name, Alice.password, "x" * 65, 100, Alice.did_symbol)
        self.assertEqual(ec, 5011, message)

        # asset description length must be less than 64.
        ec, message = mvs_rpc.create_asset(
            Alice.name, Alice.password, "x" * 64, 100, Alice.did_symbol, "x" * 65)
        self.assertEqual(ec, 5007, message)

        # secondaryissue threshold value error, is must be -1 or in range of 0
        # to 100.
        ec, message = mvs_rpc.create_asset(
            Alice.name, Alice.password, "x" * 64, 100, Alice.did_symbol, "x" * 64, rate=-2)
        self.assertEqual(ec, 5015, message)

        ec, message = mvs_rpc.create_asset(
            Alice.name, Alice.password, "x" * 64, 100, Alice.did_symbol, "x" * 64, rate=101)
        self.assertEqual(ec, 5015, message)

        # asset decimal number must less than 20.
        ec, message = mvs_rpc.create_asset(
            Alice.name, Alice.password, "x" * 64, 100, Alice.did_symbol, "x" * 64, rate=0, decimalnumber=-1)
        self.assertEqual(ec, 5002, message)
        ec, message = mvs_rpc.create_asset(
            Alice.name, Alice.password, "x" * 64, 100, Alice.did_symbol, "x" * 64, rate=0, decimalnumber=20)
        self.assertEqual(ec, 5002, message)

        # volume must not be zero.
        ec, message = mvs_rpc.create_asset(
            Alice.name, Alice.password, "x" * 64, 0, Alice.did_symbol, "x" * 64, rate=0, decimalnumber=19)
        self.assertEqual(ec, 2003, message)
        # contain sensitive words

        ec, message = mvs_rpc.create_asset(
            Alice.name, Alice.password, "JZM.xxx", 10, Alice.did_symbol, "x" * 64, rate=0, decimalnumber=19)
        self.assertEqual(ec, 5012, message)

        exist_symbol = self.getExistAssetSymbol()
        if exist_symbol:
            ec, message = mvs_rpc.create_asset(
                Alice.name, Alice.password, exist_symbol, 10, Alice.did_symbol, "x" * 64, rate=0, decimalnumber=19)
            self.assertEqual(ec, 5009, message)

    def test_2_issue_asset(self):
        ten_etp = 10 * (10 ** 8)
        # account password match error
        ec, message = mvs_rpc.issue_asset(
            Zac.name, Zac.password + '2', Zac.asset_symbol, 1)
        self.assertEqual(ec, 1000, message)

        # issue asset fee less than 10 etp
        ec, message = mvs_rpc.issue_asset(
            Zac.name, Zac.password, Zac.asset_symbol, 9)
        self.assertEqual(ec, 5006, message)

        # invalid percentage
        ec, message = mvs_rpc.issue_asset(
            Zac.name, Zac.password, Zac.asset_symbol, 10, None, 19)
        self.assertEqual(ec, 5006, message)

        ec, message = mvs_rpc.issue_asset(
            Zac.name, Zac.password, Zac.asset_symbol, 10, None, 101)
        self.assertEqual(ec, 5006, message)

        # asset symbol length must be less than 64
        ec, message = mvs_rpc.issue_asset(
            Zac.name, Zac.password, "x" * 65, ten_etp)
        self.assertEqual(ec, 5011, message)
        ...
        if exist_symbol:
            ec, message = mvs_rpc.issue_asset(
                Zac.name, Zac.password, exist_symbol, ten_etp)
            self.assertEqual(ec, 5009, message)

        # asset not in local database
        ec, message = mvs_rpc.issue_asset(
            Zac.name, Zac.password, Zac.asset_symbol, ten_etp)
        ...
        if isDomainExist(domain):
            ...
            ec, message = mvs_rpc.create_asset(
                Bob.name, Bob.password, asset_symbol, 100, Bob.did_symbol)
            self.assertEqual(ec, 0, message)

            ec, message = mvs_rpc.issue_asset(
                Bob.name, Bob.password, asset_symbol, ten_etp)
    ...
    def test_5_deletelocalasset(self):
        # account password match error
        ec, message = mvs_rpc.delete_localasset(
            Zac.name, Zac.password + '4', Zac.asset_symbol)
        self.assertEqual(ec, 1000, message)

        # asset not exist
        ec, message = mvs_rpc.delete_localasset(
            Zac.name, Zac.password, Zac.asset_symbol)
        self.assertEqual(ec, 5003, message)

        # asset not belong to Zac
        Frank.create_asset(False)
        ec, message = mvs_rpc.delete_localasset(
            Zac.name, Zac.password, Frank.asset_symbol)
        self.assertEqual(ec, 5003, message)

        Bob.create_asset(False)
        ec, message = mvs_rpc.delete_localasset(
            Bob.name, Bob.password, Bob.asset_symbol)
        self.assertEqual(ec, 0, message)

    def test_6_getaccountasset(self):
        # account password match error
        ec, message = mvs_rpc.get_accountasset(
            Zac.name, Zac.password + '5', Zac.asset_symbol)
        self.assertEqual(ec, 1000, message)
        ...
        # with +2 assets
        ec, message = mvs_rpc.create_asset(
            Bob.name, Bob.password, Bob.asset_symbol + '1', 100, Bob.did_symbol)
        self.assertEqual(ec, 0, message)
        ...
        # --cert specified
        ec, message = mvs_rpc.get_accountasset(
            Zac.name, Zac.password, cert=True)
        self.assertEqual(ec, 0, message)
        self.assertEqual(len(message), 0, message)
    ...
    def test_8_sendasset(self):
        # account password match error
        ec, message = mvs_rpc.send_asset(
            Zac.name, Zac.password + '6', "", Zac.asset_symbol, 1)
        self.assertEqual(ec, 1000, message)

        # invalid to address parameter
        ec, message = mvs_rpc.send_asset(
            Zac.name, Zac.password, "111", Zac.asset_symbol, 1)
        self.assertEqual(ec, 7006, message)

        # invalid amount parameter
        ec, message = mvs_rpc.send_asset(
            Zac.name, Zac.password, Zac.mainaddress(), Zac.asset_symbol, 0)
        self.assertEqual(ec, 5002, message)
    ...
    def test_9_sendassetfrom(self):
        ...
        ec, message = mvs_rpc.send_asset_from(
            Zac.name, Zac.password, "111", "222", Zac.asset_symbol, 1)
        self.assertEqual(ec, 7006, message)

        # invalid to address
        ec, message = mvs_rpc.send_asset_from(
            Zac.name, Zac.password, Zac.mainaddress(), "", Zac.asset_symbol, 1)
        self.assertEqual(ec, 7001, message)

        # invalid amount parameter
        ec, message = mvs_rpc.send_asset_from(
            Zac.name, Zac.password, Zac.mainaddress(), Frank.mainaddress(), Zac.asset_symbol, 0)
        self.assertEqual(ec, 5002, message)

    def test_A_burn(self):
        # account password match error
        ec, message = mvs_rpc.burn(
            Zac.name, Zac.password + '1', Zac.asset_symbol, 100)
        self.assertEqual(ec, 1000, message)
