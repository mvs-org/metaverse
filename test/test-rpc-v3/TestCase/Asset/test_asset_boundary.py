...
class TestAssetBoundary(MVSTestCaseBase):
    ...
    def test_2_issue_asset(self):
        ...
        #invalid percentage
        ec, message = mvs_rpc.issue_asset(Zac.name, Zac.password, Zac.asset_symbol, 10, None, 19)
        self.assertEqual(ec, 5006, message)

        ec, message = mvs_rpc.issue_asset(Zac.name, Zac.password, Zac.asset_symbol, 10, None, 101)
        self.assertEqual(ec, 5006, message)
