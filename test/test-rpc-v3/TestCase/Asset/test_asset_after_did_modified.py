...
class TestAssetAfterDidModified(MVSTestCaseBase):
    ...
    def test_0_scenario_did_modified(self):
        ...
        result, message = mvs_rpc.send(
            Alice.name, Alice.password, fst_did_address, 30 * 10 ** 8)
        ...
        result, message = mvs_rpc.send(
            Alice.name, Alice.password, snd_did_address, 15 * 10 ** 8)
        self.assertEqual(result, code.success, message)
        Zac.mining()
        ...
        result, message = mvs_rpc.send(
            Alice.name, Alice.password, trd_address, 15 * 10 ** 8)
        self.assertEqual(result, code.success, message)
        Zac.mining()
        ...
        ec, message = mvs_rpc.change_did(
            Zac.name, Zac.password, trd_address, snd_did_symbol)
        self.assertEqual(ec, code.success, message)
        Zac.mining()

        # test send asset
        ec, message = mvs_rpc.didsend_asset(
            Zac.name, Zac.password, snd_did_symbol, fst_asset_symbol, 3000)
        self.assertEqual(ec, code.success, message)
        Zac.mining()
        ...
        ec, message = mvs_rpc.issue_cert(
            Zac.name, Zac.password, fst_did_symbol, naming_cert_symbol, "naming")
        self.assertEqual(ec, code.success, message)
        Zac.mining()

        # test transfer cert
        ec, message = mvs_rpc.transfer_cert(
            Zac.name, Zac.password, fst_did_symbol, fst_asset_symbol, 'issue')
        self.assertEqual(ec, code.success, message)
        Zac.mining()
        ...
        ec, message = mvs_rpc.issue_asset(
            Zac.name, Zac.password, snd_asset_symbol)
        self.assertEqual(ec, code.success, message)
        Zac.mining()
