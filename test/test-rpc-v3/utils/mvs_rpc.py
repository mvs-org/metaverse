...
class RPC:
    ...
    def post(self, positional, optional):
        before = time.clock()
        rpc_rsp = requests.post(
            self.url, data=self.__to_data(positional, optional))
        after = time.clock()
        ...
        if (os.path.isfile(path)):
            ...
            if not same:
                ...
                with open(path, 'w') as f:
                    f.write(json.dumps(cls.method_keys,
            ...
            with open(path, 'w') as handle:
                handle.write(json.dumps(cls.method_keys,
                                        sort_keys=True, indent=4))
...
def mvs_api(func):
    def wrapper(*args, **kwargs):
        ...
        if "error" in rpc_rsp.json():
            print(rpc_rsp.json()['error']['code'],
                  rpc_rsp.json()['error']['message'])
...
def easy_new_account(account, password):
    return "getnewaccount", [account, password], {}, lambda result: (result["mnemonic"].split()[-1], result["default_address"])


@mvs_api
def get_account(account, password, lastword):
    return "getaccount", [account, password, lastword], {}, lambda result: (result["mnemonic"], result["address_count"])


@mvs_api
def new_address(account, password, address_count=1):
    return "getnewaddress", [account, password], {'--number': address_count}, None


@mvs_api
def list_addresses(account, password):
    return "listaddresses", [account, password], {}, None
...
def get_publickey(account, password, address):
    return "getpublickey", [account, password, address], {}, lambda result: result["public_key"]
...
def eth_submit_work(nonce, header_hash, mix_hash):
    ...
    // Request
    curl -X POST --data '{"jsonrpc":"3.0", "method":"eth_submitWork", "params":["0x0000000000000001", "0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef", "0xD1GE5700000000000000000000000000D1GE5700000000000000000000000000"],"id":73}'
