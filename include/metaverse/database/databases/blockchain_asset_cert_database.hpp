...
public:
    ...
    std::shared_ptr<chain::asset_cert> get(const hash_digest& hash) const;

    /// Get all asset certs
    std::shared_ptr<std::vector<chain::asset_cert>> get_blockchain_asset_certs() const;

    void store(const chain::asset_cert& sp_cert);
