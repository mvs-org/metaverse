...
public:
    ...
    std::shared_ptr<asset_mit_register_info> get(const hash_digest& hash) const;

    /// Get all asset certs
    std::shared_ptr<asset_mit_register_info::list> get_blockchain_mits() const;

    void store(const asset_mit_register_info& mit_info);
